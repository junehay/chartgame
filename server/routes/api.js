...
router.get('/buy', async (req, res, next) => {
  try {
    ...
    const gameData = await getRedisData(uuid, 'gameData');
    const jsonData = JSON.parse(gameData);
    const nowPrice = jsonData['chart'][49].price[3]; // chartData.length : 80, [{"price":[option, low, start, end, high, tooltip],"volume":[option, value]}, ...]
...
router.get('/ranklist', async (req, res, next) => {
  ...
  const rankList = recordData.map(e => {
    let data = {};
    data._id = e.id;
    data.name = e.name;
