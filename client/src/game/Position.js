...
const Position = () => {
  ...
  const buyPrice = useRecoilValue(buyPriceState);
  const stocks = useRecoilValue(stocksState);
  const gainPrice = useRecoilValue(gainPriceState);
  const gainPercent = useRecoilValue(gainPercentState);
  const winCount = useRecoilValue(winCountState);
  const loseCount = useRecoilValue(loseCountState);
  const accGainPrice = useRecoilValue(accGainPriceState);
        ...
        <Table>
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell align="left" style={{ paddingBottom: 5 }}>
                평가금액
              </TableCell>
              <TableCell align="center" style={{ paddingBottom: 5 }}>
                {(nowPrice * stocks).toLocaleString()}원
              </TableCell>
            </TableRow>
          </TableBody>
        </Table>
        ...
        <Table>
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell align="center" style={accStyle}>
                {accGainPrice.toLocaleString()}
              </TableCell>
              <TableCell align="center" style={accStyle}>
                {(((100000000 + accGainPrice) / 100000000 - 1) * 100).toFixed(2)}%
              </TableCell>
